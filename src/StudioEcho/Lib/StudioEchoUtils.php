...
class StudioEchoUtils
{
     ...
     */
    public static function cleanupSlugPart($slug, $replacement = '-')
    {
        // transliterate
        ...
        // remove accents resulting from OSX's iconv
        $slug = str_replace(array('\'', '`', '^'), $replacement, $slug);
    ...
    /**
     *  Genere le slug "à la Propel" à partir de la chaine fournie
     *
     * @param   string $txt
     *
     * @return    string               the slugified text
     */
    public static function generateSlug($txt)
    {
        $toSlug =  \StudioEcho\Lib\StudioEchoUtils::transliterateString($txt);
        $slug = \StudioEcho\Lib\StudioEchoUtils::cleanupSlugPart($toSlug);
        return $slug;
    }
