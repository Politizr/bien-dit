...
abstract class BasePOEmail extends BaseObject implements Persistent
{
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if ($this->aPOrder !== null) {
                if (!$this->aPOrder->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aPOrder->getValidationFailures());
                }
            }

            if ($this->aPOOrderState !== null) {
                if (!$this->aPOOrderState->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aPOOrderState->getValidationFailures());
                }
            }

            if ($this->aPOPaymentState !== null) {
                if (!$this->aPOPaymentState->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aPOPaymentState->getValidationFailures());
                }
            }

            if ($this->aPOPaymentType !== null) {
                if (!$this->aPOPaymentType->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aPOPaymentType->getValidationFailures());
                }
            }

            if ($this->aPOSubscription !== null) {
                if (!$this->aPOSubscription->validate($columns)) {
                    $failureMap = array_merge($failureMap, $this->aPOSubscription->getValidationFailures());
                }
            }


            if (($retval = POEmailPeer::doValidate($this, $columns)) !== true) {
